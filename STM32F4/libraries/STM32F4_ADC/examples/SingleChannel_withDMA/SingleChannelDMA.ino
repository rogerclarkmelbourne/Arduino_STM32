/*
  Single channel analog input stored in circular buffer example.
  The sampling rate is given by timer 3 update frequency.
  At each timer overflow the DMA will store the ADC value to buffer.
...
const uint8 adcPin = PA0;

#define maxSamples  200
uint16_t buffer[maxSamples]; // stores the values sampled by ADC
...
#define ADC_DEVICE_DUAL   1 // use this to activate both ADC1 and 2
                            // The buffer will store 32 bits per sample.
...
void setup()
{
	...
	Serial.println("START Single channel at sample rate circular buffer example");
...
void loop()
{
	if ( readPtr!=NULL )
	{
		...
		// process data - send to serial
		for (uint16_t i=0; i<(maxSamples/2); i++ )
		{
			Serial.write('x'); Serial.print((secondPart*(maxSamples/2))+i);
